...
class RevokeAccessDataTable extends Component {
  ...
  tab = () => {
    const {
      isLoadingUser, selected, onCheckBoxChange, undoDecision, onUpdateStatus, onToggleComment, getFilterData
    } = this.props;
    const columns = [
      {
        title: '',
          ...
          const menu = (
            <Menu>
              ...
              <Menu.Item><span
                ...
                onClick={() => undoDecision(record.id)}>Undo decision</span></Menu.Item>
            </Menu>
          );
